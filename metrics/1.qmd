...
-   Where $\epsilon_i$ is the error term function - that determines the error for each unit $i$. This error term essentially reflects the "distribution" that we discussed earlier.
:::

As I noted before, we need to estimate intercept $\beta_0$ and slope $\beta_1$ before we can use our model. This will be covered in the next section.

Once we have the estimates of $\beta_0$ and $\beta_1$, what can we do with this model?

1.  Causal inference and estimation: given a few (important) conditions that have to be met, $\beta_1$ can be interpreted as the causal effect of $X$ on $Y$ (or the average treatment effect). We can also use the regression model to estimate the average treatment effect of binary $D$ treatment variables, and randomised controlled trials.
