...
-   Mixed strategies will be denoted with a $\sigma$ instead of an $s$.
...
Formally, the strategy $s_i$ is a best response to the opponents specific strategy $s_{-i}$, if $u_i(s_i, s_{-i})≥ u_i(s_i', s_{-i})$, where $s'_i$ are the other strategy profiles of player $i$ that are not the chosen $s_i$
...
-   Pareto efficiency isn't about which outcomes are plausible - it just looks at which strategies could be considered the "best" for all players. The outcomes will be determined by our equilibrium concepts, such as Nash Equilibrium.
...
-   The group can get the best outcome (pareto efficient) if they all cooperate.
...
In theory, all Nash Equilibria are supposed to be equally likely to occur - there is no mathematical proof that one would occur more frequently than others. However, there are two ways in which one Nash Equilibria may stand above the rest, and be the most likely outcome of the game.

First, if one Nash Equilibria is Pareto efficient, when the others are not, then that one is likely to be the outcome.

-   This is because all players know the payoffs, since that is common knowledge, and all players are able to calculate the Nash Equilibria.
...
Second, is a concept proposed by Schelling: Focal Points.
...
Have you ever noticed that many similar businesses are often located in the same place? There are often competing petrol stations all around the same intersection. Or multiple fast food or coffee chains in the same place. Or a bunch of Chinese stores concentrated in one small area.
...
In the Downsian Model, we assumed that there are only 2 parties and candidates. What if there were more than 2 parties (as is frequently the case)? [In a 3 party election, there would no longer be a Nash Equilibrium at the median voter position]{.underline}. This is because any party $i$ has a profitable deviation in this scenario.
