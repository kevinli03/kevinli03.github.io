...
2.  **Game Theory and Formal Models** (This Book) introduces key concepts in the field of Game Theory, which has become a very popular tool to model and study political situations. This book starts off with Static Games, before moving to Dynamic Games and Bayesian Games.
3.  **Empirical Political Economy** applies the econometric and game theory techniques in the previous two books to the study of current issues in Political Science and Political Economy.
...
Note: This book requires a familiarity with some single variable calculus and probability (specifically, a strong understanding of Bayes' Rule). See [Mathematical Methods for Political Economy](https://kevinli03.github.io/politicaleconomy/maths.pdf).
...
## Downsian Models of Electoral Competition
...
### Evaluating Median Voter Theory

Median Voter Theory predicts that both political parties converge towards the median voter. Is this the case in reality?

-   You probably have heard of the complaint: *"I am not voting, because both parties are essentially the same thing"*. This might be used as evidence supporting Median Voter Theory. A more tangible real world example is the difference between Tony Blair/Gordon Brown and David Cameron, both centrist political figures.

-   However, you might also point to the United States, where we have seen a massive divergence between the parties. Donald Trump and Kamala Harris certainly do not seem to be converging, although they do spend a lot of money trying to flip swing states with many "median voters".
...
Let us look at the assumptions of the Median Voter Theory:

1.  **Bounded and One Dimensional**: Median Voter Theory assumes the policy space is both bounded between -1 and 1, as well as being one dimensional (left-right only, no authoritarian/libertarian and progressive/conservative axis, or other forms of political compass).
2.  **Distribution of Citizens**: Median Voter Theory assumes that citizens are distributed uniformly along the spectrum. What happens if a society has polarised citizens, or if the citizens lean in one direction?
3.  **Symmetrical Preferences**: Median Voter Theory assumes that the only thing affected citizen's choice is the distance between their reference and the party, which implies differences in both directions (right and left) are equal in magnitude for the voter.
4.  **No Cost of Voting and No Abstaining**: Median Voter Theory assumes no costs of voting, and that everyone does vote. What happens if this is not the case?
5.  **Two Party System**: Median Voter Theory assumes there are only 2 parties. What happens when a 3rd or more enter?
6.  **Parties are Rent-Seeking**: Median Voter Theory assumes parties only aim to win elections - they do not care about their actual policies, they only use that to win an election. But what if politicians do care about the policies they are promoting?
7.  Parties can propose any policy, and can commit to implementing these policies - for example, even if Donald Trump promised to become communist, the voters would believe him. If they did not, then that would mean parties cannot freely switch their position on the spectrum to win votes, and the candidate's own background matters more. This introduces the **Citizen-Candidate Model**.

In the rest of the chapter, we will go through each assumption, and see how altering each assumption may affect our predictions.
