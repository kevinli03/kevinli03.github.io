---
title: "Empirical Political Economy"
subtitle: "Volume I: Econometric Methods"
...
**Empirical Political Economy** is a series that goes hand-in-hand with the series: [An Introduction to Political Economy](https://kevinli03.github.io/politicaleconomy/1.pdf), which introduces the concept of using economic models to study political phenomena. This series focuses on empirical techniques in which we can use real world data to test our Political Economy models.

The Empirical Political Economy series has three volumes:

-   **Volume I: Econometric Methods** (this book) focuses on the standard econometric/statistical toolkit of economists and political scientists. We cover standard topics such as statistical inference and sampling, linear regression, randomised experiments, and causal techniques for observational studies.

-   [**Volume II: Further Econometric Methods**](https://kevinli03.github.io/econometrics/2.pdf) extends the techniques learned in the first volume to deal with more complicated questions in Political Economy. Topics include other types of regression, time series analysis, multivariate dimensional reduction, latent variable modelling, and structural equation models.

-   [**Volume III: Machine Learning Methods**](https://kevinli03.github.io/econometrics/3.pdf) introduces more modern advances in empirical Political Economy, including non-linear and tree-based prediction methods, classification, model validation and regularisation, and quantitative text analysis.
...
In the social sciences, we are often interested in studying large groups of people and entities. However, it is often impossible to ask every single individual in the population. For example, if we wanted to study the average educational level of the UK, we would need to ask nearly 70 million people.

A **sample** is a subset of a population, which ideally, can tell us something about the population. If our sample can reflect the population, then we can use the sample in our study, instead of the larger population.

**Sampling** is the process by which we select a sample from a larger population, and we want the sample to be representative of the population. [The quality of a sample depends on two major factors: the sampling procedure, and luck]{.underline}.

[The gold standard of sampling procedure is a **random sample**]{.underline} - where individuals in the sample are selected at random from the population. In a random sample, every possible individual has an equal chance of being selected, and thus, the resulting sample is likely to be reflective of the common traits of the population.

### Central Limit Theorem

Remember how we explained that the quality of a sample also depends on luck? Well, Central Limit Theorem helps account for the luck aspect.

Before we introduce the Central Limit Theorem, we need to explain a **distribution of sample means**.

-   Imagine that we take a sample from a population. Then, we find the mean of the variable we are interested in the sample. That is a **sample mean**.
...
[The **Central Limit Theorem** states that the distribution of sample means of a variable, will be approximately normally distributed.]{.underline}
...
![](figures/1/3.1.png){fig-align="center" width="70%"}
...
-   Note: If $Y$ is binary (i.e. only 0 or 1), we interpret it as the [expected change in probability of outcome category $Y=1$ occurring]{.underline}.
...
We essentially treat the explanatory variable as a variable of 2 different categories, category $X_j=0$, and category $X_j=1$

Now, [$\hat{\alpha}$ is the expected value of $Y$ for an observation in category $X_j = 0$]{.underline}.

[$\hat{\beta}_j$ is the expected difference in the value of $Y$, between the categories $X_j = 1$ and $X_j = 0$]{.underline}

[To find the expected value of $Y$ for an observation in $X_j=1$, you would need to do $\hat{\alpha} + \hat{\beta}_j$.]{.underline}

You can test these interpretations yourself by plugging in $X=0$ and $X=1$ into a linear model.

### Model Summary Statistics

#### Estimated Residual Standard Deviation

We can derive the estimate of the **residual variance** $\sigma^2$ (variance of the error term) with this formula (standard deviation is the square root of it):
...
The residual standard deviation $\hat{\sigma}$ is consistent throughout a model. This is one of the assumptions of the linear regression model - that errors are consistently distributed, no matter the value of $X$. This assumption is called **homoscedasticity**.
...
Our estimation so far has produced a $\hat{\beta}_j$ based on the data in our sample. However, remember, the sample is just a fraction of the population - and the true $\hat{\beta}_j$ is unlikely to be exactly the one in our sample. If this confuses you, see Sections 3.1 and 3.2.
...
-   $\hat{se}(\hat{\beta}_j)$ is the standard error of our estimate of how precisely we have estimated the true value of $\beta_j$. Smaller values indicate more precise estimates, and generally occur when either residual variance is low $\hat{\sigma}^2$, or when sample size $n$ is large. We let the computer solve for this.

-   The $\approx 1.96$ means approximately 1.96, however, this will slightly deviate depending on the sample size and number of variables (do not worry, the computer will solve this for us). This is because this value is taken from a $t$ distribution, which depends on the degrees of freedom $DF = n - k - 1$.

[We are 95% confident, that the true value of $\beta_j$ is within this interval]{.underline}. Every value in the interval is a plausible value of the true $\beta_j$ in the population.
...
Like I mentioned previously, generally, for regressions, our hypotheses that we test are:
