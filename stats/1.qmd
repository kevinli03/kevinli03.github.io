...
The goal of econometrics is to use data to answer economic questions. These economic questions often revolved around human decision making and social settings, in which traditional statistical methods did not work well for, or were not possible to implement. Political science has similar issues to economics, and thus, has adopted many econometric techniques.
...
Descriptive inference is about learning about some relationship in the world. This might seem very simple: just measure the relationship! However, the biggest issue is with how large populations are.

-   For example, if we want to see the relationship between individual income and likelihood to vote in the UK, we would have to ask every single voter in the UK, likely over 50 million individuals. Researchers do not have the time or money to do this!
...
An important thing to know is that [correlation does not equal causation]{.underline}:
...
**Causal States** are these hypothetical states of the world. The [control state]{.underline} is the world where a unit does not receive the treatment $D$. The [treatment state]{.underline} is identical to the control state, with the only exception that a unit receives the treatment $D$.
...
Thus, the **individual causal effect** $\tau$ of the treatment $D$ for any unit $i$ must be $\tau_i = Y_{1i} - Y_{0i}$. This is because since the two states of the world are identical except for treatment $D$, the resulting difference must be as a result of treatment $D$.
...
### Naive Estimator and Sample Bias
...
2.  The second part $E[Y_{0i} | D_i = 1] - E[Y_{0i} | D_i = 0]$ is what we call the **sample bias**. Intuitively, it is the difference between the treatment and control groups prior to the treatment taking place (hence the potential outcome being $Y_{0i}$).
...
\frac{\partial S(\hat{\alpha}, \hat{\beta})}{\partial \hat{\alpha}} \left[ \sum\limits_{i=1}^n (Y_i - \hat{\alpha} - \hat{\beta}X_i)^2 \right] = \frac{\partial S(\hat{\alpha}, \hat{\beta})}{\partial \hat{\alpha}} \left[ (Y_1 - \hat{\alpha} - \hat{\beta}X_i)^2 \right] + ... + \frac{\partial S(\hat{\alpha}, \hat{\beta})}{\partial \hat{\alpha}} \left[ (Y_n - \hat{\alpha} - \hat{\beta}X_i)^2 \right]
$$
