...
subtitle: "Book 1 of the Econometrics Sequence in An Introduction to Political Economics"
...
This book is designed to be both an approachable, but also rigorous, introduction to Econometrics, specifically for the analysis of political institutions and actors. I assume a basic understanding of statistics, up to correlation. I also assume familiarity with calculus and some linear algebra. To see what is specifically required, consult the Quantitative Methods sequence (particularly the first book).

This book will also use the R language for some calculations, so a basic understanding of the language is useful - however, this is not needed if you just want an understanding of the methods.

### Introduction to Causal Inference

Causal inference is the primary focus of econometrics and the social sciences. An important thing to note is that [correlation does not equal causation]{.underline}:
...
More mathematically, the naive estimator is: $$
...
We have some fitted model to our data (best-fit line) with estimated parameter values: $$
...
\frac{\partial S(\hat{\alpha}, \hat{\beta})}{\partial \hat{\alpha}} \left[ (Y_i - \hat{\alpha} - \hat{\beta}X_i)^2 \right] = -2(Y_i - \hat{\alpha} - \hat{\beta}X_i)
$$

That is the derivative of one of the items in the summation. But how do we deal with the summation? We know that there is the sum rule of derivatives $[f(x) + g(x)]' = f'(x) + g'(x)$. Thus, we know: $$
\frac{\partial S(\hat{\alpha}, \hat{\beta})}{\partial \hat{\alpha}} \left[ \sum\limits_{i=1}^n (Y_i - \hat{\alpha} - \hat{\beta}X_i)^2 \right] = \frac{\partial S(\hat{\alpha}, \hat{\beta})}{\partial \hat{\alpha}} \left[ (Y_1 - \hat{\alpha} - \hat{\beta}X_1)^2 \right] + ... + \frac{\partial S(\hat{\alpha}, \hat{\beta})}{\partial \hat{\alpha}} \left[ (Y_n - \hat{\alpha} - \hat{\beta}X_n)^2 \right]
$$
...
Now that we have the partial derivative of the SSE function in respect to $\hat{\alpha}$, to find the value of $\hat{\alpha}$ that creates the minimum value of the SSE, we set the first order derivative equal to 0. We can ignore the -2, since if the sum is equal to 0, then the -2 will have no effect. $$
\begin{split}
& -2 \sum\limits_{i=1}^n (Y_i - \hat{\alpha} - \hat{\beta}X_i) = 0 \\
& \Longrightarrow \sum\limits_{i=1}^n (Y_i - \hat{\alpha} - \hat{\beta}X_i) = 0
\end{split}
$$
...
Now, let us find the minimum $\hat{\beta}$ value by taking the partial derivative in respect to $\hat{\beta}$ and setting it equal to 0. This is almost the same as before - use chain rule, then use sum rule to get the deriative: $$
\begin{split}
...
-   Covariance: $\sigma_{XY} = \sum\limits_{i=1}^n (X_i - \bar{X})(Y_i - \bar{Y})$

-   Variance: $\sigma_X^2 = \sum\limits_{i=1}^n(X_i - \bar{X})^2$

Thus, $\hat{\beta}$ could also be rewritten as (but only for bivariate linear regression): $$
\hat{\beta}_1 = \frac{Cov(X, Y)}{Var(X)} = \frac{\sigma_{XY}}{\sigma_x^2}
$$
...
There are a few properties of the OLS estimator that we can explore directly from this process of estimation.
...
We can plug $\hat{u}_i$ in to get a few properties of the OLS estimator.
...
This means that with the OLS estimator, the total sum of the residuals for all units $i$ is 0. Furthermore, since $\hat{u}_i = Y_i - \hat{Y}_i$, we know that the average $Y_i$ should be equal to the average $\hat{Y}_i$.
...
This tells us that the sample covariance/correlation between the regressor $X$ and the residual $\hat{u}_i$ is always zero. Or in other words, there is no correlation/relationship between $X$ and the residuals we see in our model.
...
Another property of the OLS estimator is that the explanatory variable $X$ must have variation. Why? Recall the final formula for the $\hat{\beta}$ estimation that we came up with:

$$
\hat{\beta}_1 = \frac{Cov(X, Y)}{Var(X)} = \frac{\sigma_{XY}}{\sigma_x^2}
$$

Thus, if $Var(X) = 0$, then our $\hat{\beta}_1$ will be undefined, as we are dividing by zero.

Thus, for OLS to be possible, $Var(X) ≠ 0$ must be true.

### Multivariate Regression Estimation

Multivariate regression, as introduced previously, allows us to add additional control variables into our model as follows:
...
Since the -2 is multiplied, we can ignore it, since if the rest of the sums are equal to 0, the 02 is irrelevant. Thus, our minimisation criteria is:

$$
\begin{split}
& \sum\limits_{i=1}^n (Y_i - \hat{\alpha} - \hat{\beta}_1X_{1i} -  \hat{\beta}_2X_{2i}) = 0 \\
& \sum\limits_{i=1}^n X_{1i}(Y_i - \hat{\alpha} - \hat{\beta}_1X_{1i} -  \hat{\beta}_2X_{2i}) = 0 \\
& \sum\limits_{i=1}^n X_{2i} (Y_i - \hat{\alpha} - \hat{\beta}_1X_{1i} -  \hat{\beta}_2X_{2i}) = 0 \\
& \text{and so on for } X_{3i}, ..., X_{ki}
\end{split}
$$
...
The other conditions are much more difficult to solve by hand, so the computer will do this for us. However, it is still valuable to know these first order conditions to know what the computer is trying to find.
