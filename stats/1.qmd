...
-   However, in this scenario, we are not comparing the potential outcomes of the same individual. We are comparing the outcome of some observation $A$ in treatment $Y_{1A}$ and the outcome of some other observation $B$ in control $Y_{1B}$.
...
Simple Linear Regression is a special case of the linear model, when there is only one explanatory variable $X$. How do we interpret parameters $\hat{\alpha}$ and $\hat{\beta}$ that we have calculated? Let us first focus on a descriptive context - we will focus on the causal interpretation later.
...
### Linear Algebra Depiction of Multivaraite Regression
