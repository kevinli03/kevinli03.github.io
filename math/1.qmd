...
How to get rid of $n$ to isolate $x$? We know that $n \times (1/n) = 1$. We also know that $(a^x)^y = a^{xy}$. Thus: $$
\begin{split}
& (x^n)^{(1/n)} = a^{(1/n)} \\
...
Note that in statistics, when we say $\log(x)$, we are talking about natural logs (base $e$). We can change the base of logarithms:
...
Or a more general rule with constant $c$:$$
\begin{split}
...
-   A partial derivative is what we will need to find.
...
What is the difference between a local and global extrema? Well, a local exterma only has to be the maximum/minimum in its "neighbourhood", meaning there can be multiple local extrema. Global extrema are a subset of local extrema. There can only be one global maximum or minimum in a function.
...
![](figures/1/3.6.png){fig-align="center" width="50%"}
...
![](figures/1/3.7.jpeg){fig-align="center" width="50%"}
...
Integrals, also called anti-derivatives, are the opposite of derivatives.
...
Note: there are many indefinite integrals $F$, for any given function $f$. This is because when we take a derivative, we lose any constants, since $\frac{d}{dx} [c] = 0$. Thus, many different functions $F$ with different $c$, when taking the derivative, will result in function $f$.
