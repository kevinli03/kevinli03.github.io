...
3.  [Volume III: Advanced Statistical Techniques](https://kevinli03.github.io/math/3.pdf) covers logit and count regression models, time series models, and latent variable models. This builds on the core sequence book: Econometrics for Political Analysis.
4.  [Volume IV: Data Science Techniques](https://kevinli03.github.io/math/4.pdf) introduces modern data science and statistical learning techniques that have improved prediction accuracy. This builds on the core sequence book: Econometrics for Political Analysis.
5.  [Volume V: Proof and Analysis](https://kevinli03.github.io/math/5.pdf) covers introductory proofs in mathematics. Useful, but not essential for the rest of the Political Economics.

The 1st volume is a pre-requisite for the main series on Political Economics, and covers the core techniques in calculus, probability, and statistics needed. The 2nd volume is more advanced topics that are useful, but not formally required. Volumes 3-5 are advanced topics that are not required, but can be useful and interesting to those who enjoy Political Economics
...
-   For example, $A = \{1,2,3,4,5\}$, or, $A=\{n|n \in \mathbb{Z}; 1 ≤ n ≤ 5\}$ - or in other words, $A$ is the set of values $n$, *such that* $n$ is in the set of all *integers* $Z$, and $n$ is between 1 and 5 inclusive.
...
Sometimes, you will have an equation that takes the form $x^n = a$, where $n$ is some number. To solve for $x$, the solution is $x = a^{(1/n)}$.
...
The zeroes/roots of a function are the input values of the function that make the output function equal to 0. Mathematically, for what $x$ does $f(x) = 0$. We can also think about this graphically. Since output $f(x)$ is graphed on the $y$-axis, we can see at what $x$-values the function cross the line $y=0$ (the $x$-axis line).
...
Essentially, we calculate the output value of $g(x)$, and immediately input that value into $f(x)$. For example, if $h(x) = x^2$, and $f(x) = x-5$, then $f \circ h = (x^2) - 5$
...
A limit is when a function approaches some specific output value $y=f(x) = L$, as the input $x$ moves closer and closer to some value $c$. Or more intuitively, as the $x$ value becomes infinitely closer to $x=c$, the $y=f(x)$ value becomes infinitely close to $f(x) = L$. Limits are notated with the form: $\lim\limits_{x \rightarrow c} f(x) = L$

For example, the figure below shows a function with a limit:

![](figures/1/1.3.jpeg){fig-align="center" width="35%"}
...
The simplest way to do this is to find the inverse function (as shown in section 1.5), and calculate the derivative of that. However, the function $x = f^{-1}(y)$ is not always easily differentiable with the rules we have introduced. We can utilise an alternative approach. Remember the identity of inverse functions shown in section 1.5:
...
Suppose that we have a function $f : X_1, X_2, ..., X_n \rightarrow \mathbb{R}$. Or in other words, a function with more than one variable input. However, we might only be interested in the rate of change (slope) relative to only one of the variables.

We can do this with a partial derivative, which is defined as:
...
To solve a partial derivative, we take the derivative in respect to our specific variable $x_i$, while treating all other input variables $x_1, ..., x_n$ like a constant, like you would treat the number 5.
...
Tkate the function $f: A \rightarrow \mathbb{R}$, in which $A$ is domain of $f$ such that $A \subseteq \mathbb{R}$.
...
What is the difference between a local and global extrema? Well, a local exterma only has to be the maximum/minimum in its "neighbourhood", meaning there can be multiple local extrema. Global extrema are considered a subset of local extrema. There can only be one global maximum or minimum in a function.

### Optimisation and Finding Extrema

Optimisation is about finding the maximum or minimum point of a function. This is useful because we often want to optimise things in statistics and models - whether it be minimise error, or maximise utility, and so on.
...
![](figures/1/3.6.png){fig-align="center" width="55%"}

Thus, knowing these facts, we can identify local extrema of a function $f(x)$ in an interval with the following steps:
...
However, we are not done with checking extrema - we have to calculate the edge cases. These are the end-points of our interval. Why? - look at the following figure below - on the interval $[-3, 7]$, clearly $x=7$ will have a larger maximum value than any local maximum, since the function tends towards infinity as $x$ approaches infinity:

![](figures/1/3.7.jpeg){fig-align="center" width="55%"}
