...
![](figures/econometrics/2.1.png){fig-align="center" width="60%"}
...
![](figures/econometrics/2.2.png){fig-align="center" width="70%"}
...
![](figures/econometrics/3.1.png){fig-align="center" width="80%"}
...
-   The **response variable** (dependent variable) is notated $Y$. There is only one response variable throughout this course - multivariate models with multiple $Y$ are covered in the Advanced Econometrics for Political Economy book.
...
In formal statistical terminology, a regression model is the specification of the conditional distribution of $Y$, given $\overrightarrow{X}$. Essentially, it is stating that the distribution of possible $Y$ outcomes depends on the value of $\overrightarrow{X}$.
...
The linear regression model focuses on the **expected value** of the conditional distribution of $Y$ given $\overrightarrow{X}$. Essentially, the model outputs the expected value/mean of the distribution of $Y$ given any set of $\overrightarrow{X}$.
...
-   Where $E[Y_i|\overrightarrow{X}_i]$ is the expected value of the conditional distribution of some $Y$ value $Y_i$, given some $\overrightarrow{X}$ values $\overrightarrow{X}_i$.
...
The above form of the linear model is solved for the expected value of the conditional distribution $Y_i|\overrightarrow{X}_i$. However, we can also solve for the value of any point $Y_i$ in our data:
...
In our model, we have parameters $\alpha, \beta_1, ..., \beta_k$ and variance $\sigma^2$ that need to be estimated, in order to create a best-fit line we can actually use for prediction and statistical inference. Our best estimates of these parameters will be denoted with a hat: $\hat{\alpha}, \hat{\beta}_1, ...$

Estimating the parameters is also called **fitting the model**. We estimate the parameters by using our observed data points $(Y_i, \overrightarrow{X}_i)$, and fitting a best fit line to these points. Our result should take the following form:
...
However, how do we determine the best estimates of our parameters $\alpha, \beta_1, ..., \beta_k$? There are actually several ways to do this, including Bayesian Linear Regression. We will focus on the most popular method: **The Ordinary Least Squares Estimator (OLS).**

OLS estimation attempts to **minimise the sum of squared errors** of our predicted line to our actual observed data. The estimation processes is as follows.

1.  First, we propose some coefficient values to test. Let $\tilde{\beta}$ represent the vector of our proposed coefficient values $\tilde{\alpha}, \tilde{\beta}_1, ..., \tilde{\beta}_k$
2.  Then, we use these proposed coefficients in our prediction line: $\hat{Y}_i (\tilde{\beta}) = \tilde{\alpha} + \tilde{\beta}_1 X_{1i},...  ,\tilde{\beta}_k X_{ki}$
...
Of course, testing every possible set of proposed coefficients $\tilde{\beta}$ is quite time-consuming. Luckily, through calculus and linear algebra (which we will not explain here), you can derive the parameters that minimise the SSE of a simple linear regression with a formula.

For a simple linear regression with one $X$ variable, the formula for $\hat{\beta}$ is as follows:
...
We can also derive the estimate of the **residual variance** $\sigma^2$ with this formula:
