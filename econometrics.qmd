...
$\hat{\beta}$ is the slope of the the linear model. More formally, [$\hat{\beta}$ is the expected change in $Y$, given a one-unit increase in $X$]{.underline}. Or in other words, as $X$ increases by 1 unit, the expected value $Y$ increases by $\hat{\beta}$ units.
...
Then, we have to consult a t-distribution. If we recall from Section 2.4, the t distribution is a distribution that is similar to a normal distribution in shape (just with a less pronounced peak and thicker tails). Notably, the t-distribution only has one parameter - degrees of freedom, which is calculated by the number of observations $n$, minus the number of variables $k$, then minus 1: $DF = n - k -1$
